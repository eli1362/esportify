    ...
    try {
        ...
        if ($check->rowCount() > 0) {
            ...
            if (!$event) {
                ...
                // Insère un message dans la table session_messages
                $message = "L'utilisateur {$_SESSION['username']} s’est inscrit à l'événement (en attente de validation).";
                $msg_stmt = $pdo->prepare("INSERT INTO session_messages (event_id, user_id, message, created_at) VALUES (:event_id, :user_id, :message, NOW())");
                $msg_stmt->execute([
                    'event_id' => $event_id,
                    'user_id' => $user_id,
                    'message' => $message
                ...
                // ✅ Message to display in dashboard_user
                $_SESSION['message'] = "En attente de validation ⏳";
            }
        }
